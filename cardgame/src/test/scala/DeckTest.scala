...
class DeckSuite extends AnyFunSuite {
  ...
  test("Dealing from the deck works correctly") {
    ...
    assert(card.obtainCardType === CardType.Ten)
    ...
    assert(card.obtainCardType === CardType.Eight)
    assert(deck.size === CardType.values.size - 2)
    assert(deck.toString === expected)

    for i <- 1 to 9 do  // skip to second last card
      deck.dealCard()

    // deal second last card in deck
    card = deck.dealCard()
    expected = "Three"
    assert(card.obtainCardType === CardType.Six)
    assert(deck.size === 1)
    assert(deck.toString === expected)
    ...
    card = deck.dealCard()
    expected = ""
    assert(card.obtainCardType === CardType.Three)
    assert(deck.size === 0)
    assert(deck.toString === expected)
  }
